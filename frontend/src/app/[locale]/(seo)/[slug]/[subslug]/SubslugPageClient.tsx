...
import { getLocalizedName, getCategoryName } from "@/lib/catalog-types";
...
export function SubslugPageClient(props: SubslugPageClientProps) {
  if (props.type === "discipline") {
    return <DisciplineCityView {...props} />;
  }
  return <CategoryCityView {...props} />;
}

function DisciplineCityView({
  discipline,
  cityName,
  citySlug,
  locale,
  ...
  const faqItems = [
    {
      key: "cost",
      question: t("faqCost", {
        discipline: catName.toLowerCase(),
        city: cityName,
        defaultValue: "Ile kosztują treningi {discipline} w {city}?",
      }),
      answer: t("faqCostAnswer", {
        discipline: catName.toLowerCase(),
        defaultValue:
          "Ceny za treningi różnią się w zależności od instruktora, lokalizacji i długości sesji. Na Exercio znajdziesz profile z widocznym cennikiem — możesz porównać oferty i wybrać najlepszą dla siebie.",
      }),
    },
    {
      key: "choose",
    ...
    },
    {
      ...
      answer: t("faqFirstAnswer", {
        discipline: catName.toLowerCase(),
        defaultValue:
          "Pierwsza konsultacja to zazwyczaj rozmowa o Twoich celach, stanie zdrowia i oczekiwaniach. Instruktor dobiera odpowiedni plan treningowy i pokazuje podstawowe ćwiczenia.",
      }),
    },
  ];
          ...
          <div className="flex flex-col items-start gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl">
              <div className="mb-2 flex items-center gap-3">
                ...
                <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">
                  {locale === "pl"
                    ? `${catName} w ${cityName} — znajdź instruktorów i kluby`
                    : `${catName} in ${cityName} — find instructors and clubs`}
                </h1>
              </div>
              <p className="text-base text-slate-400">
                {locale === "pl"
        ...
        {disciplines.length > 0 && (
          <div className="mb-12">
            <h2 className="mb-4 text-xl font-bold text-white">
              {locale === "pl"
                ? `Style tańca w kategorii ${catName}`
                : `Dance styles in ${catName}`}
            </h2>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {disciplines.map((discipline) => {
                const discName = getLocalizedName(discipline.names, locale);
                return (
                  <Link
                    key={discipline.id}
                    href={`/${locale}/${discipline.slugs[locale === "pl" ? "pl" : "en"]}/${citySlug}`}
                    className="flex items-center gap-3 rounded-lg border border-slate-700/50 bg-slate-900/50 p-4 transition-colors hover:border-orange-500/50 hover:bg-slate-800/50"
                  >
                    <span className="text-2xl" aria-hidden="true">
                      {discipline.icon}
                    </span>
                    <div>
                      <span className="font-medium text-white">{discName}</span>
                      <p className="text-sm text-slate-400">
                        {locale === "pl"
                          ? `Sprawdź instruktorów ${discName.toLowerCase()} w ${cityName}`
                          : `Check ${discName.toLowerCase()} instructors in ${cityName}`}
                      </p>
                    </div>
                  </Link>
                );
          ...
          <div className="mb-12">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-white">
                  {t("results", {
                    defaultValue: `Dostępni specjaliści w {city}`,
                    discipline: catName,
                    city: cityName,
                  })}
                </h2>
                <p className="mt-1 text-sm text-slate-400">
                  {t("resultsDescription", {
                    ...
                    city: cityName,
                    defaultValue:
                      "Znaleźliśmy {count} specjalistów w {city} z tej kategorii. Wszyscy posiadają publiczne profile oraz możliwość kontaktu.",
                  })}
                </p>
              </div>
              <Link
