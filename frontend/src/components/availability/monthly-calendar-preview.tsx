...
export function MonthlyCalendarPreview() {
  ...
  const getDaySlotsWithDetails = (day: Date): TimeSlot[] => {
    ...
    return slotsQuery.data
      .filter(
        (slot) => format(new Date(slot.startTime), "yyyy-MM-dd") === dateKey,
      )
      .map((slot) => ({
        time: format(new Date(slot.startTime), "HH:mm"),
        available: slot.available,
  ...
  const selectedMobileDayData = processedSlotsMap.get(
    format(selectedMobileDay, "yyyy-MM-dd"),
  );
  const selectedMobileDaySlots: ProcessedDaySlots["slots"] =
    selectedMobileDayData?.slots ?? [];
  const selectedMobileHasException =
    selectedMobileDayData?.hasException ?? false;
  ...
  return (
    <div className="bg-slate-900/50 rounded-lg border border-slate-700 p-3 sm:p-4">
      ...
      {/* Error State - check if bookings are disabled */}
      {slotsQuery.isError && (
        <div className="text-center py-12" role="alert">
          {!instructorProfile?.isBookingEnabled ? (
            <div className="space-y-3">
              <p className="text-slate-200 text-base font-medium">
                {t("bookingsDisabled")}
              </p>
              <p className="text-slate-400 text-sm">
                {t("bookingsDisabledHint")}
              </p>
            </div>
          ) : (
          ...
          <div className="hidden md:block">
            <div className="grid grid-cols-7 gap-0.5 sm:gap-1 overflow-hidden">
              ...
              {(locale === "pl"
                ...
                <div
                  key={day}
                  className="text-center text-[10px] sm:text-xs font-medium text-slate-400 py-1.5 sm:py-2"
                >
                  {day}
                </div>
              ))}
              ...
              {/* Calendar Days */}
              {daysInMonth.map((day) => {
                const dayData = processedSlotsMap.get(
                ...
                return (
                  ...
                  >
                    <div
                      className={`text-[10px] sm:text-xs font-medium mb-0.5 sm:mb-1 text-center ${
                        dayIsPast
                          ? "text-slate-600"
                          : hasException
                            ? "text-purple-400"
                      ...
                      <div className="space-y-0.5 max-h-16 sm:max-h-20 overflow-hidden">
                        ...
                        {slots.length > 3 && (
                          <div
                            ...
                            title={t("moreSlotsTooltip", {
                              count: slots.length - 3,
                            })}
                          >
                            +{slots.length - 3}
                          </div>
                  ...
                  selectedMobileDaySlots.map((slot, idx) => {
                    ...
                    return (
                      <button
                        key={idx}
                        onClick={() =>
                          isClickable && handleDayClick(selectedMobileDay)
                        }
