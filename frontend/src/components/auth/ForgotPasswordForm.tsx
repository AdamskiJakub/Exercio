...
export function ForgotPasswordForm() {
  ...
  const { form, isLoading, error, success, onSubmit } = useForgotPasswordForm();
      ...
      {success ? (
        <div className="text-center space-y-4">
          <div className="bg-green-500/10 border border-green-500/50 rounded-lg p-4">
            <p className="text-green-400 text-sm">{t('checkYourEmail')}</p>
            <p className="text-slate-400 text-xs mt-2">{t('resetCodeSent')}</p>
          </div>
        </div>
      ) : (
