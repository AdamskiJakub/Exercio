...
export type PageHeaderVariant = 'gradient-orange' | 'gradient-full' | 'white-accent' | 'white';
...
interface PageHeaderProps {
  ...
  className?: string;
  /** Show decorative blur background */
  showBlur?: boolean;
}
...
export function PageHeader({
  ...
  const variantStyles = {
    'gradient-orange': 'bg-linear-to-r from-orange-500 via-red-500 to-orange-600 bg-clip-text text-transparent',
    'gradient-full': 'bg-linear-to-r from-orange-500 to-red-500 bg-clip-text text-transparent',
    ...
    return (
      <div className={cn('space-y-4', className)}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className={cn('text-4xl font-bold mb-2', variantStyles[variant])}>
              {title}
            </h1>
            {subtitle && (
    ...
    return (
      <div className={cn('relative mb-8 text-center', className)}>
        ...
        <div className={containerAlign[align]}>
          <h1 className={cn('text-4xl md:text-5xl font-bold mb-3', variantStyles[variant])}>
            {title}
          </h1>
          {subtitle && (
  ...
  // Standard layout
  return (
    <div className={cn('space-y-3', alignStyles[align], containerAlign[align], className)}>
      <h1 className={cn('text-4xl md:text-5xl font-bold', variantStyles[variant])}>
        {title}
      </h1>
      {subtitle && (
        <p className="text-slate-400 text-lg md:text-xl">
